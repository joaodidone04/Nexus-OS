...
import CartoesModule       from "./modules/CardsModule.jsx";
import InvestimentosModule from "./modules/InvestimentosModule.jsx";
...
export default function FinanceStation() {
  ...
  const [cards,            setCards]            = useStorage("nx-finance-cards",        []);
  const [cardTxs,          setCardTxs]          = useStorage("nx-finance-cardtxs",      []);
  const [investAccounts,   setInvestAccounts]   = useStorage("nx-finance-invest-accs",  []);
  const [investMovements,  setInvestMovements]  = useStorage("nx-finance-invest-movs",  []);
  ...
  const grantXp = useCallback((ruleKey) => {
    ...
    if (fbAction) award(fbAction).catch(() => {});
    ...
    <div className="fs-root">
          ...
          {MODULES.map(m => (
            ...
            >
              <span className="fs-nav-icon" style={{ color: active === m.id ? m.color : undefined }}>
                ...
                {m.id === "cartoes"       && <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>}
      ...
      <main className="fs-main">
        <div className="fs-main-inner">
          {active === "dashboard"     && <DashboardModule   transactions={transactions} accounts={accounts} goals={goals} cards={cards} cardTxs={cardTxs} investAccounts={investAccounts} investMovements={investMovements} />}
          ...
          {active === "investimentos" && <InvestimentosModule investAccounts={investAccounts} setInvestAccounts={setInvestAccounts} investMovements={investMovements} setInvestMovements={setInvestMovements} onXp={grantXp} />}
          {active === "cartoes"       && <CartoesModule     cards={cards} setCards={setCards} cardTxs={cardTxs} setCardTxs={setCardTxs} />}
